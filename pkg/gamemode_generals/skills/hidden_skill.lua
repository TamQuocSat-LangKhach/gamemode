...
---@type TrigFunc fun(self: TriggerSkill, event: TriggerEvent, target: ServerPlayer?, player: ServerPlayer, data: MaxHpChangedData|any): any
local hidden_skill_on_use = function(self, event, target, player, data)
  ...
  if event == fk.BeforeMaxHpChanged then
    ...
    if Fk.generals[player:getMark("__hidden_general")] then -- 耦合了
