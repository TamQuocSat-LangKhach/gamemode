...
fenyue:addEffect('active', {
  ...
  on_use = function(self, room, effect)
    ...
    if pindian.results[target.id].winner == player then
      if player:prohibitUse(Fk:cloneCard("slash")) or player:isProhibited(target, Fk:cloneCard("slash")) or room:askToChoice(player, {choices = {"vd__fenyue_slash", "vd__fenyue_prohibit"}, skill_name = fenyue.name}) == "vd__fenyue_prohibit" then
...
local vd_fenyue_prohibit = fk.CreateSkill {
  name = "#vd__fenyue_prohibit"
}

vd_fenyue_prohibit:addEffect('prohibit', {
...
fenyue:addRelatedSkill(vd_fenyue_prohibit)
